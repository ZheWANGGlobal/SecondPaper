...
/*static void testRealtime(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	if(toolbox == NULL)
		mexErrMsgTxt("hCRF: No toolbox was previously created. Use function 'createToolbox' to create a toolbox.\n");
	if(data == NULL)
		mexErrMsgTxt("hCRF: No dataset was previously loaded. Use function 'loadData' to load a dataset.\n");
	if(nrhs<1)
		mexErrMsgTxt("hCRF: No buffer length (or delay) provided.\n");
	...
	{
		if(!mxIsEmpty(prhs[0]))
			delay = (int)*((double*)mxGetData(prhs[0]));
	}

	if(nrhs > 1)
	{
		pOutput = strOutput;
		int r = mxGetString(prhs[1], strOutput, sizeof(strOutput));
		if(r!=0) 
		{
			mexErrMsgTxt("hCRF: Ouput filename must be a string.\n");
			pOutput = 0;
	...
	{
		pStats = strStats;
		int r = mxGetString(prhs[2], strStats, sizeof(strStats));
		if(r!=0) 
		{
			mexErrMsgTxt("hCRF: Stats filename must be a string.\n");
			pStats = 0;
	...
	toolbox->testRealtime(*data, delay, pOutput, pStats);
}*/
...
static void set(int nlhs, mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	{
		if(!mxIsEmpty(prhs[1]))
			toolbox->setRandomSeed(*((long*)mxGetData(prhs[1])));
	}
    ...
    else if(!strcmp(strNameParameter,"alphaReg"))
    {
        //alpha reg L2 didn't really prove useful.
        toolbox->getModel()->setAlphaRegL1(*((double*)mxGetData(prhs[1])));
    }
	else
		mexErrMsgTxt("hCRF: Unknown parameter.\n");
...
static void addFeatureFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
	...
	if(!strcmp(name,"RawFeatures"))
		...
		id = EDGE_FEATURE_ID;
	else if(!strcmp(name,"WindowRawFeatures"))
		id = WINDOW_RAW_FEATURE_ID;
	else if(!strcmp(name,"GaussianWindowRawFeatures"))
		id = GAUSSIAN_WINDOW_RAW_FEATURE_ID;
	else if(!strcmp(name,"BackwardWindowRawFeatures"))
		id = BACKWARD_WINDOW_RAW_FEATURE_ID;
	else if(!strcmp(name,"LabelEdgeFeatures"))
		id = LABEL_EDGE_FEATURE_ID;
	else if(!strcmp(name,"SquareRawFeatures"))
		id = SQUARE_RAW_FEATURE_ID;
	else if(!strcmp(name,"FeaturesOne"))
		id = ONE_FEATURE_ID;
	else if(!strcmp(name,"SharedFeatures"))
		id = LATENT_LABEL_FEATURE_ID;
	else if(!strcmp(name,"EdgeObservationFeatures"))
		id = EDGE_OBSERVATION_FEATURE_ID;
	else if(!strcmp(name,"StartFeatures"))
		id = START_FEATURE_ID;
	else if(!strcmp(name,"GateNodeFeatures"))
...
static void setInferenceEngine(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
	...
	if(!strcmp(type,"BP"))
		engine = new InferenceEngineBP();
	else if(!strcmp(type,"Brute"))
		engine = new InferenceEngineBrute();
	else if(!strcmp(type,"DC"))
		engine = new InferenceEngineDC();
	...
	else if(!strcmp(type,"FF"))
	{
		if (nrhs > 1)
		{
			int delay = (int)*((double*)mxGetData(prhs[1]));
			engine = new InferenceEngineFF(delay);
		}
		else
			mexErrMsgTxt("hCRF: InferenceEngineFF missing a parameter (int). Need a delay length for buffer simulation.\n");
	}
	...
	//TODO: See why Perceptron is not deriving from InferenceEngine like the others.
	/*else if(!strcmp(type,"Perceptron"))
		engine = new InferenceEnginePerceptron();*/
#endif
...
static void setOptimizer(int nlhs, mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	if(r == 0)
	{
		if(!strcmp(strOptimizer,"cg")||!strcmp(strOptimizer,"CG"))
			opt = OPTIMIZER_CG;
		else if(!strcmp(strOptimizer,"bfgs")||!strcmp(strOptimizer,"BFGS"))
			opt = OPTIMIZER_BFGS;
#ifndef _PUBLIC
		else if(!strcmp(strOptimizer,"asa")||!strcmp(strOptimizer,"ASA"))
			opt = OPTIMIZER_ASA;
		else if(!strcmp(strOptimizer,"owlqn")||!strcmp(strOptimizer,"OWLQN"))
			opt = OPTIMIZER_OWLQN;
		else if(!strcmp(strOptimizer,"nrbm")||!strcmp(strOptimizer,"NRBM"))
			opt = OPTIMIZER_NRBM;
#endif
		else if(!strcmp(strOptimizer,"lbfgs")||!strcmp(strOptimizer,"LBFGS"))
			opt = OPTIMIZER_LBFGS;
		else
...
static void setGradient(int nlhs, mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	if(toolbox == NULL)
		mexErrMsgTxt("hCRF: No toolbox was previously created. Use function 'createToolbox' to create a toolbox.\n");

	if(nrhs < 1)
		mexErrMsgTxt("hCRF: setGradient: At least one input parameter: Gradient type (string).\n");
	...
	{
#ifndef _PUBLIC
		if(!strcmp(str,"GradientCNF"))
			grad = GRADIENT_CNF;
		else if(!strcmp(str,"GradientLDCNF"))
			grad = GRADIENT_LDCNF;
#endif
		else
			mexErrMsgTxt("hCRF: Invalid gradient string. Only gradients supported for now are CNF and LDCNF, others are present by default in their respective toolboxes, so you don't need to set them.\n");
	}
	toolbox->setGradient(grad);
}
