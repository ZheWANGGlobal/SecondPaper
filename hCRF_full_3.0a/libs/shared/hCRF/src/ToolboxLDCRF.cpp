...
void ToolboxLDCRF::openPort(PORT_NUMBER portNumber, int windowSize, int bufferLength)
{
	if(portNumberMap.find(portNumber) != portNumberMap.end())
		delete portNumberMap[portNumber];
	portNumberMap[portNumber] = new DataSequenceRealtime(windowSize, bufferLength, 
		pModel->getNumberOfRawFeaturesPerFrame(), pModel->getNumberOfStates() ); 
}

void ToolboxLDCRF::closePort(PORT_NUMBER portNumber)
{
	if(portNumberMap.find(portNumber) != portNumberMap.end())
	{
		DataSequenceRealtime *dataSequenceRealtime = portNumberMap[portNumber];
		if(dataSequenceRealtime != NULL)
			delete dataSequenceRealtime;
		portNumberMap.erase(portNumber);
	}
}

bool ToolboxLDCRF::insertOneFrame(PORT_NUMBER portNumber, const dVector* const features, dVector* prob)
{
	DataSequenceRealtime* dataSequence = portNumberMap[portNumber];
	dataSequence->push_back(features);
	bool ready = dataSequence->isReady();

	//If the box is full, ready to infer.
	if(ready)
	{
		pInferenceEngine->computeSingleBelief(pFeatureGenerator, pModel, 
				dataSequence, prob);
	}
	return ready;
}
