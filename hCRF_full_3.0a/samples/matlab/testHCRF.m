...
matHCRF('createToolbox',modelHCRF.modelType, modelHCRF.nbHiddenStates); %{+KGB}
if isfield(modelHCRF,'windowSize')
   matHCRF('addFeatureFunction','WindowRawFeatures',modelHCRF.windowSize);
end
matHCRF('initToolbox');
...
if isfield(modelHCRF,'outlierTresh')
    seqs = removeOutliers(seqs, modelHCRF.outlierTresh(2), modelHCRF.outlierTresh(1));     
end

for i = 1:size(seqs, 2)
    if ~isfield(modelHCRF,'windowRecSize') || size(seqs{i},2) <= modelHCRF.windowRecSize
        subSeq = seqs(i);
