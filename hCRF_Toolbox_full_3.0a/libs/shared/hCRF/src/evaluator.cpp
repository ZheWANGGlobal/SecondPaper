...
double Evaluator::computeError(DataSet* X, Model* m, bool bComputeMaxMargin)
{
...
#pragma omp parallel shared(std::cout, X, m, NumIters, bComputeMaxMargin, error)	\
	private(TID) \
	default(none)
	{
#ifdef _OPENMP
...
#pragma omp critical(output)
			...
			error += this->computeError(X->at(i), m, bComputeMaxMargin) * X->at(i)->getWeightSequence();
		}
	}
								   ...
								   bool bComputeMaxMargin)
{
	...
	Beliefs bel;
	pInfEngine->computeBeliefs(bel, pFeatureGen, X, m, 0, -1, false, bComputeMaxMargin);
