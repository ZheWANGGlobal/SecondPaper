...
double GradientFD::computeGradient(dVector &vecGradient, Model *m, DataSequence *X, bool bComputeMaxMargin)
{
	if( bComputeMaxMargin )
		throw HcrfNotImplemented("GradientFD for max-margin is not implemented");
...
// Selection of h from Numerical Recipes in C++, page 392
double GradientFD::computeGradient(dVector &vecGradient, Model *m, DataSet *X, bool bComputeMaxMargin)
{
	// get pointer to weight parameter vector
	dVector oldWeights;
	oldWeights.set(*m->getWeights());
