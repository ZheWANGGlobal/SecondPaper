...
double Gradient::computeGradient(dVector& vecGradient, Model* m, DataSet* X, bool bComputeMaxMargin)
{
...
#else
#pragma omp parallel \
	shared(vecGradient, X, m, ans, nbFeatures, bComputeMaxMargin, std::cout)	\
	private(TID) \
	default(none)
#endif
	{
...
#pragma omp critical(output)
			...
			localSum += computeGradient(tmpVecGradient, m, x, bComputeMaxMargin) * x->getWeightSequence();
			if(x->getWeightSequence() != 1.0)
				tmpVecGradient.multiply(x->getWeightSequence());
			localGrads[TID].add(tmpVecGradient);
#else
			localSum += computeGradient(localGrads[TID], m, x, bComputeMaxMargin);// * x->getWeightSequence();
#endif
		}
...
////////////////////////////////////////////////////////////
	...
	if(!m->isNRBM() && sigmaL2Square != 0.0f) {
		if (m->getDebugLevel() >= 2){
