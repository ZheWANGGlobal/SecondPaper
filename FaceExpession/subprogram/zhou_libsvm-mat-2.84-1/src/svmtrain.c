...
int read_problem_dense(const mxArray *label_vec, const mxArray *instance_mat)
{
	...
	num_y =mxGetN(label_vec);
	if (num_y >= 2)
	{
		prob.cobj = Malloc(double, prob.l);
		prob.alpha0 = Malloc(double,prob.l);
		prob.Aeq = Malloc(double, prob.l);
	}
	...
	for(i = 0; i < prob.l; i++)
	{
		...
		if (num_y == 4)
		{
			prob.cobj[i] = labels[prob.l+i];
			prob.alpha0[i] = labels[2*prob.l+i];
			prob.Aeq[i] = labels[3*prob.l+i];
			
		}
		else if (num_y == 3)
		{
			prob.Aeq[i] = prob.cobj[i] = labels[prob.l+i];
			prob.alpha0[i] = labels[2*prob.l+i];
		}
		else if (num_y == 2)
		{
